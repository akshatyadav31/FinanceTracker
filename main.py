...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.logo_label = ttk.Label(self.sidebar_frame, text="Finance Tracker", font=("Arial", 20, "bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        # Adding navigation buttons with custom colors
        self.income_button = ttk.Button(self.sidebar_frame, text="Income", command=self.income_button_event, style="Navigation.TButton")
        self.income_button.grid(row=1, column=0, padx=20, pady=10)
        self.expenses_button = ttk.Button(self.sidebar_frame, text="Expenses", command=self.expenses_button_event, style="Navigation.TButton")
        self.expenses_button.grid(row=2, column=0, padx=20, pady=10)
        self.balance_button = ttk.Button(self.sidebar_frame, text="Balance", command=self.balance_button_event, style="Navigation.TButton")
        self.balance_button.grid(row=3, column=0, padx=20, pady=10)

        # Create Entry fields(User can type the text) with custom colors
    ...
    def create_transaction_frame(self, title, button_command, row, transactions):
        frame = ttk.Frame(self.main_frame, style="Transaction.TFrame")
        ttk.Label(frame, text=f"Add {title}", font=("Arial", 16)).grid(row=0, column=0, columnspan=2)
        
        ttk.Label(frame, text="Category", style="Label.TLabel").grid(row=1, column=0)
        category_entry = ttk.Entry(frame)
        category_entry.grid(row=1, column=1)

        ttk.Label(frame, text="Amount", style="Label.TLabel").grid(row=2, column=0)
        amount_entry = ttk.Entry(frame)
        amount_entry.grid(row=2, column=1)

        ttk.Button(frame, text=f"Add {title}", command=lambda: button_command(category_entry, amount_entry), style="Add.TButton").grid(row=3, column=0, columnspan=2)
    ...
    def create_balance_frame(self, row):
        frame = ttk.Frame(self.main_frame, style="Balance.TFrame")
